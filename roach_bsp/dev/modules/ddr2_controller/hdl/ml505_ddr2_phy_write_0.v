...
//  \   \         Application: MIG
//  /   /         Filename: ml505_ddr2_phy_write_0.v
...
module ml505_ddr2_phy_write_0 #
  ...
  always @(*)
    wr_stages[0] <= (phy_init_data_sel)
                    ? ctrl_wren : phy_init_wren;
    ...
    if (ECC_ENABLE) begin:gen_ecc_reg
      always @(posedge clk90) begin
        if(phy_init_data_sel) begin
          wdf_data_r <= wdf_data;
          wdf_mask_r <= {&wdf_mask_data[16:9], wdf_mask_data[16:9],
                         &wdf_mask_data[7:0], wdf_mask_data[7:0]};
        end else begin
